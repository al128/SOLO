/* _$ SOLO.JS Common web & canvas utilities MIT License https://github.com/al128/SOLO http://therook.co.uk http://belowthestorm.co.uk */
...
String.prototype.monitor = function() {
  ...
  jQuery(document).keydown(function(e) {
    jQuery("body").trigger("io.keydown." + that);
  }).keyup(function(e) {
    jQuery("body").trigger("io.keyup." + that);
  });
};
...
String.prototype.splice = function(idx, rem, s) {
  /* Insert text into string at index */
...
(function(jq) {
  ...
  /* Social (facebook & twitter) helpers */
  jq.fb = {};jq.twitter = {};
  ...
  jq.twitter.share = function(text, url) {
    ("https://twitter.com/intent/tweet?text=mytexthere&url=myurl").goTo();
  }

  /* Create SOLO extension to store data & caches */
  jq._solo = {
    cache:{},
    settings: {
      "debug":{"lastmessage":""},
    ...
    io: {
      ...
      init: function(mouse_el, mouseonly) {
        jq._solo.tick.init();
        jq._solo.io.mouse = {"start":null, left : 0, moved : false};
        jq._solo.io.touch = {"start":null, down : 0, moved : false};
        jq._solo.io.keys = {};
        jq._solo.io.update();
        if (!mouse_el) mouse_el = jq(document);

        mouse_el.on("mousedown", function(e) {
          jq.getMousePosition(e);
          e.preventDefault();
          switch(e.button) {
            case 0:
              jq._solo.io.mouse.left = 1;
              jq._solo.io.mouse.start = {x: _$.mx, y: _$.my};
              jq._solo.io.mouse.moved = false;
              break;
            case 1:
          ...
          jq.fireSmartEvent("io.left.down", e);
        });
        mouse_el.on("mousemove", function(e) {
          jq.getMousePosition(e);
          if (jq._solo.io.mouse.start) {
            jq._solo.io.mouse.moved = true;
            jq.fireSmartEvent("io.left.move", e);
          }
        });
        mouse_el.on("mouseup", function(e) {
          jq.getMousePosition(e);
          e.preventDefault();
          switch(e.button) {
            case 0:
              jq._solo.io.mouse.left = -1;
              jq._solo.io.mouse.start = null;
              break;
            case 1:
              jq._solo.io.mouse.middle= -1;
              break;
          ...
          jq.fireSmartEvent("io.left.up", e);
        });

        mouse_el.on("touchstart", function(e) {
          jq._solo.io.touch.start = jq.getMousePosition(e);
          jq._solo.io.touch.down = 1;
          jq._solo.io.touch.difference = 0;
          jq._solo.io.touch.distance = 0;
          jq.fireSmartEvent("io.touch.down", e);
        });
        mouse_el.on("touchmove", function(e) {
          var fingerOne = jq.getMousePosition(e);
          if (jq._solo.io.touch.start) {
            if (e.originalEvent.touches.length > 1) {
              var fingerTwo = jq.getCursorPosition(e, 1);
              var distance = jq.lineDistance(fingerOne.x, fingerOne.y, fingerTwo.x, fingerTwo.y);
              if (jq._solo.io.touch.distance !== 0) jq._solo.io.touch.difference = jq._solo.io.touch.distance - distance;
              jq._solo.io.touch.distance = distance;
            }
            jq._solo.io.touch.moved = true;
            jq.fireSmartEvent("io.touch.move", e);
          }
        });
        mouse_el.on("touchend", function(e) {
          jq._solo.io.touch.start = null;
          jq._solo.io.touch.down = -1;
          jq._solo.io.touch.difference = 0;
          jq._solo.io.touch.distance = 0;
          jq.fireSmartEvent("io.touch.up", e);
        });
      ...
      update: function() {
        ...
        _$.io.keys.action1 = 0;
        _$.io.keys.action2 = 0;
        if (_$.io.mouse.left === -1)    _$.io.mouse.left = 0;
        if (_$.io.mouse.middle === -1)  _$.io.mouse.middle = 0;
        if (_$.io.mouse.right === -1)   _$.io.mouse.right = 0;
        if (_$.io.touch.down === -1)    _$.io.touch.down = 0;
      },
      active : false
    },
    log: function(message, e) {
      /*  We want to log and catch as much as we can to easily identify errors and important system messages and states */
      if (!this.settings.debug) return;
      if (message === this.settings.debug.lastmessage) return;
      ...
      this.settings.debug.lastmessage = message;
    },
    tick: {
      active: false,
  ...
  jq.extend(jq.fn, {
    get2d: function() {
      ...
      try {
        ...
        if (_$.detection.ie8) {
          ...
          if (ctx.globalCompositeOperation == 'color' && !_$.detection.isSafari) {
    ...
    safeClick: function(callback) {
      /* "Safe" because it prevents unwanted propagation */
    ...
    resizeEl: function(w, h) {
      this.attr("width", w).attr("height", h);
    },
    centerEl: function(offset,container,margin) {
      this.each(
        function(i){
          if (!offset) offset = 0;
  ...
  jq.lineDistance = function(x1, y1, x2, y2) {
    var xs = x2 - x1;
    xs = xs * xs;
    var ys = y2 - y1;
    ys = ys * ys;
  ...
  jq.fireSmartEvent = function(event, one, two, three) {
    if (!event) return;
    jq("body").trigger(event, [one, two, three]);
  };
  jq.registerSmartEvent = function(event, callback) {
    if (!event || !callback) return;
    jq("body").on(event, function(e, _one, _two, _three) {
      callback(e, _one, _two, _three);
    });
  }
  ...
  jq.getCursorPosition = function(e, finger) {
    var rect = jq(e.target).getOffset();
    if (!finger) finger = 0;
  ...
  jq.getMousePosition = function(e) {
    var results = jq.getCursorPosition(e);
    if (!results) return;
    _$.mx = results.x;
    _$.my = results.y;
    if (jq._solo.io.active) {
      jq._solo.io.mouse.x = _$.mx;
      jq._solo.io.mouse.y = _$.my;
      jq._solo.io.touch.x = _$.mx;
      jq._solo.io.touch.y = _$.my;
    }
    return results;
  };
  ...
  jq.random = function(max, min) {
    if (!min) min = 1;
    return Math.floor(Math.random()*(max-min+1)+min);
  };
  jq.registerUpdate = function(callback, name) {
    if (!name) name = "tick";
    if (name === "tick") jq._solo.tick.init();
  ...
  jq.doBrowserDetection = function() {
    if (_$.detection) return;
    _$.detection = {};
    _$.detection.touch = 'ontouchstart' in window || 'onmsgesturechange' in window;
    _$.detection.isOpera = !!window.opera || navigator.userAgent.indexOf('Opera') >= 0;
    _$.detection.isFirefox = typeof InstallTrigger !== 'undefined';
    _$.detection.isSafari = Object.prototype.toString.call(window.HTMLElement).indexOf('Constructor') > 0;
    _$.detection.isChrome = !!window.chrome;
    _$.detection.isIE = /*@cc_on!@*/false;
    ...
    _$.detection.platform = navigator.platform;
    _$.detection.isMac = false;
    _$.detection.isWindows = false;
    try {
      if (_$.detection.platform.toLowerCase().indexOf("mac")) {
        _$.detection.isMac = true;
      } else if (_$.detection.platform.toLowerCase().indexOf("win")) {
        _$.detection.isWindows = true;
      }
    } catch (e) {
      _$.log("Couldn't detect platform");
    }
  };
