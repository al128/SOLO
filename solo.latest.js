/* SOLO web app utils MIT License | github.com/al128/SOLO | therook.co.uk | belowthestorm.co.uk REQUIRES JQUERY */
...
String.prototype.goTo = function(tab) { if (tab === false) { window.location = this; } else { window.open(this, '_blank'); } };
...
/* Fire event */
String.prototype.eventFire = function(e, one, two, three) { jQuery("body").trigger(this, [e, one, two, three]); };

/* Register event */
String.prototype.eventRegister = function(callback) {
  if (!callback) return;
  jQuery("body").on(this, function(e, _one, _two, _three) {
    callback(e, _one, _two, _three);
  });
};

/* Retrive a value from page url */
String.prototype.queryString = function() { var key = this.replace(/[*+?^$.\[\]{}()|\\\/]/g, "\\$&"); var match = window.location.search.match(new RegExp("[?&]"+key+"=([^&]+)(&|$)"));  return match && decodeURIComponent(match[1].replace(/\+/g, " ")); };
...
/* Random number */
Math.rand = function(max, min) { if (!min) min = 1; return Math.floor(Math.random()*(max-min+1)+min); };
...
Math.collides = function(a, b) { return a.x < b.x + b.width && a.x + a.width > b.x && a.y < b.y + b.height && a.y + a.height > b.y; };
...
Object.prototype.removeFromArray = function(index, amount) { if (!amount) {amount = 1;} this.splice(index, amount);};
...
/* Deep copy */
Object.prototype.deepCopy = function() { return jQuery.extend(this, {}, null); };
...
function MousePad(el) {
  ...
  function getCursorPosition(e, finger) {
    var rect = jQuery(e.target).getOffset();
    if (!finger) finger = 0;
  ...
  el.on("mousedown", function(e) {
    ...
    switch(e.button) {
      case 0:
        ...
        distance = 0;
        moved = false;
		"io.left.down".eventFire(e);
        break;
      case 1:
        middle = 1;
		"io.middle.down".eventFire(e);
        break;
      case 2:
        right = 1;
		"io.right.down".eventFire(e);
        break;
    }
  });
  ...
  el.on("mouseup", function(e) {
    ...
    switch(e.button) {
      case 0:
        left = -1;
        start = null;
		"io.left.up".eventFire(e);
        break;
      case 1:
        middle= -1;
		"io.middle.up".eventFire(e);
        break;
      case 2:
        right = -1;
		"io.right.up".eventFire(e);
        break;
    }
  });
...
function Keyboard(el) {
  ...
  jQuery(document).keydown(function(e) { getKey(e, true); }).keyup(function(e) { getKey(e, false); });
...
(function(jq) {
  jq.extend(jq.fn, {
    get2d: function() {
      ...
      if (ctx.globalCompositeOperation == 'color' && !"safari".isBrowser()) {
        jq("html").addClass("colorblending");
      } else {
        jq("html").addClass("no-colorblending");
      }
      return cx.getContext('2d');
    },
  ...
  /* Shortcuts */
  jq.onReady = function(callback) { jq(document).ready(function(){callback();}); };
  jq.onResize = function(callback) { jq(window).resize(function(){callback();}); };
}(jQuery));
$ = jQuery;
...
if (typeof(CanvasRenderingContext2D) !== "undefined") {
  ...
  CanvasRenderingContext2D.prototype.toGrayScale = function() {return this.changeSaturation(-1);};
  ...
  CanvasRenderingContext2D.prototype.clearBlend = function() { this.globalCompositeOperation = 'source-over'; };
