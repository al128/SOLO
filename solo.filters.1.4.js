...
var SoloFilterBank = function() {
  ...
  /* Filters */	
	...
	filters.applyblend = function(pixels, val, context) {
		if (val) context.applyBlend(val);
		return pixels;
	}
  ...
  function init(s) {
		...
		for (var i = 0; i < runnings.length; i++) {			
			...
			if (runnings[i].indexOf(":") > -1) {
				...
				obj.id = running[0].trim();
				switch (obj.id) {
					case 'applyblend':
							obj.value = running[1].trim();
						break;
					default:
							obj.value = parseFloat(running[1].trim());
						break;
				}
			} else {
